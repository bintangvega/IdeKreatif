            ...
            <div class="card mb-4">
                <div class="card-body">
                    ...
                    <!-- Dropdown untuk memilih kategori -->
                     <div class="mb-3">
                     <lable for="category_id" class="form-label">Kategori</label>
                     <select class="form-select" name="category_id" required>
                        <!-- Mengambil data kategori dari database untuk mengisi opsi dropdown -->
                         <option value="" selected desable> Pilih Aalah satu</option>
                         <?php
                         $query = "SELECT * FROM categories"; // Query ubtuk manembil data kategori
                         $result = $conn->query($query); // Menjalankan query
                         if ($result->num_rows > 0) { // Jika terdapat data kategori
                            while ($row = $result->fetch_assoc()) { // Itelari setiap kategori
